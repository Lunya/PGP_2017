...
export class AccountComponent implements OnInit {
	...
	ngOnSubmit() {
		...
		this.http.patch(url + '/' + this.currentUser.id, this.accountForm.value, { headers: headers }).subscribe((result: any) => {
			if (result.error) {
				this.loading = false;
			} else {
				console.log(result);
	...
	deleteAccount() {
		...
		let headers = new HttpHeaders();
		headers = this.auth.addAuthHeader(headers);
		this.http.delete(url + '/' + this.currentUser.id, {headers: headers}).subscribe((result: any) => {
			if (result.error) {
				this.loading = false;
			} else {
				console.log(result);
